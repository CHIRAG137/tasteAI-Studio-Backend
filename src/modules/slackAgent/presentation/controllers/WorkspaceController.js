...
class WorkspaceController {
  ...
  getOAuthUrl = async (req, res) => {
    ...
    }&scope=chat:write,chat:write.customize,commands,users:read,channels:read,channels:join,groups:write,app_mentions:read,channels:history,groups:history,im:history,reactions:read&user_scope=&redirect_uri=${encodeURIComponent(
      redirectUri,
    )}&state=${state}`;
  ...
  list = async (req, res) => {
    const workspaces = await this.slackAgentFacade.listWorkspacesUseCase.execute(
  ...
  getById = async (req, res) => {
    const workspace = await this.slackAgentFacade.getWorkspaceInfoUseCase.execute(

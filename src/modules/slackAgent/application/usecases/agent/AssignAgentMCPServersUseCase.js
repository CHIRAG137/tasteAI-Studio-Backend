  ...
  async execute(command) {
    ...
    if (mcpServerIds && mcpServerIds.length > 0) {
      for (const mcpId of mcpServerIds) {
        ...
        if (!server) throw new Error(`MCP server ${mcpId} not found`);
      }
    }

    const updated = await this.agentRepository.update(agentId, { mcpServerIds: mcpServerIds || [] });
    await this.auditService.log('agent.mcp_servers_assigned', { agentId, mcpServerIds, organizationId, actorId });
    return updated;
  }
}
